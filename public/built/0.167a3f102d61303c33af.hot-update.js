	...
	var Controller = (function () {
	    function Controller($scope, fxAction) {
	        ...
	        $scope.$watch($scope.model, function (newValue, oldValue) {
	            if (!$scope.boost && newValue) {
	                $scope.boost = new Builder(form ? form : $scope.form, fxAction, $scope.model);
	                var onChange_1 = $scope.boost.onChange.bind($scope.boost);
	                $scope.boost.onChange = function (item) {
	                    $scope.ngModel.$setViewValue(onChange_1(item));
...
//# sourceMappingURL=0.167a3f102d61303c33af.hot-update.js.map