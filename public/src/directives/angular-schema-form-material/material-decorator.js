angular.module("schemaForm").run(["$templateCache", function($templateCache) {
    ...
    $templateCache.put("decorators/material/array.html", "<div class=\"schema-form-array {{form.htmlClass}}\" sf-field-model=\"sf-new-array\" sf-new-array=\"\"><md-toolbar class=\"md-default md-hue-2\"><div class=\"md-toolbar-tools\"><h2 flex=\"\">{{ ::form.title }}</h2><md-button ng-hide=\"form.readonly || form.add === null\" ng-click=\"appendToArray()\" class=\"md-icon-button {{ form.style.add || \'btn-default\' }}\"><md-tooltip>新增</md-tooltip><md-icon><ng-md-icon icon=\"add_circle_outline\"></ng-md-icon></md-icon></md-button></div></md-toolbar><md-content ng-model=\"modelArray\" flex=\"\" layout-padding=\"\" class=\"md-whiteframe-1dp sortable-container\" sv-root=\"\" sv-part=\"modelArray\"><md-list><md-list-item layout=\"row\" sv-element=\"{{form.sortOptions || {containment:\'.sortable-container\'} }}\" ng-repeat-start=\"item in $$value$$ track by $index\" sf-field-model=\"ng-repeat-start\" form=\"copyWithIndex($index)\" schema-form-array-items=\"\"><md-toolbar class=\"md-hue-2\" sv-placeholder=\"\" flex=\"\"></md-toolbar><div layout=\"row\" flex-order=\"2\"><md-button ng-hide=\"form.readonly || form.add === null\" ng-click=\"appendToArray()\" class=\"md-icon-button {{ form.style.add || \'btn-default\' }}\"><md-tooltip>新增</md-tooltip><md-icon><ng-md-icon icon=\"add_circle_outline\"></ng-md-icon></md-icon></md-button><md-button ng-hide=\"form.readonly || form.remove === null\" ng-click=\"deleteFromArray($index)\" class=\"md-icon-button\" aria-label=\"Delete\"><md-tooltip>删除</md-tooltip><md-icon><ng-md-icon icon=\"close-circle-outline\"></ng-md-icon></md-icon></md-button><md-button class=\"md-icon-button md-default-theme\" sv-handle=\"\"><md-icon><ng-md-icon icon=\"cursor-move\"></ng-md-icon></md-icon></md-button></div></md-list-item><md-divider ng-repeat-end=\"\"></md-divider></md-list></md-content><md-input-container style=\"margin-top:0;\" class=\"md-block\" md-is-error=\"ngModel.$error && ngModel.$invalid && ngModel.$dirty\"><md-contact-ships class=\"ng-hide\" ng-model=\"$$value$$\" sf-field-model=\"\" sf-changed=\"form\" schema-validate=\"form\" name=\"{{::form.key|sfCamelKey}}\"></md-contact-ships><div ng-messages=\"ngModel.$error\" aria-live=\"assertive\"><div sf-message=\"\" ng-message=\"\" ng-class=\"{\'fx-invalid\':ngModel.$invalid}\"></div></div><div class=\"hint md-char-counter\" ng-if=\"form.showHints\" ng-bind=\"form.description\"></div></md-input-container></div>");
    ...
    $templateCache.put("decorators/material/checkboxes.html", "<div sf-field-model=\"sf-new-array\" sf-new-array=\"\" class=\"form-group schema-form-checkboxes {{::form.htmlClass}}\" ng-class=\"{\'has-error\': hasError(), \'has-success\': hasSuccess()}\" sf-messages=\"\"><label class=\"control-label\" ng-show=\"showTitle()\">{{::form.title}}</label><div class=\"checkbox\" ng-repeat=\"val in form.titleMap track by $index\"><md-checkbox ng-model=\"modelArray[$index]\" sf-changed=\"form\" ng-disabled=\"::form.readonly\" name=\"{{::form.key|sfCamelKey}}\" ng-true-value=\"\'{{val.value}}\'\" ng-false-value=\"\" aria-label=\"{{::form.title}}\">{{::val.name}} {{val | json}}</md-checkbox></div></div>");
    $templateCache.put("decorators/material/chips.html", "<div class=\"form-group schema-form-chips {{form.htmlClass}}\" sf-field-model=\"sf-new-array\" sf-new-array=\"\" md-no-float=\"true\" style=\"margin:1px 0;\"><label ng-show=\"form.readonly\">{{::form.title}}</label><md-chips ng-model=\"modelArray\" sf-changed=\"form\" schema-validate=\"form\" readonly=\"form.readonly\" class=\"no-errors\" placeholder=\"{{::form.title}}\"><md-chip-template><strong ng-if=\"!form.template\">{{$chip}}</strong></md-chip-template></md-chips><md-input-container class=\"md-block no-errors\" style=\"margin-top:0;\" md-is-error=\"ngModel.$error && ngModel.$invalid && ngModel.$dirty\"><div sf-message=\"\" ng-message=\"\" ng-class=\"{\'fx-invalid\':ngModel.$invalid && ngModel.$dirty}\"></div><div class=\"hint md-char-counter\" ng-if=\"form.showHints\" ng-bind=\"form.description\"></div></md-input-container></div>");
    $templateCache.put("decorators/material/date.html", "<md-input-container class=\"schema-form-date {{::form.htmlClass}}\" md-is-error=\"ngModel.$error && ngModel.$invalid && ngModel.$dirty\" sf-layout=\"\"><input sf-field-model=\"\"><md-datepicker sf-field-model=\"\" sf-changed=\"form\" schema-validate=\"form\" sf-type-parser=\"form.schema\" name=\"{{::form.key|sfCamelKey}}\" ng-show=\"::form.key\" ng-class=\"::form.fieldHtmlClass\" ng-disabled=\"::form.readonly\" md-placeholder=\"{{::form.title}}\"></md-datepicker><div ng-messages=\"ngModel.$error\" aria-live=\"assertive\"><div sf-message=\"\" ng-message=\"\"></div></div><div class=\"hint md-char-counter\" ng-if=\"form.showHints\" ng-bind=\"form.description\"></div></md-input-container>");
    ...
    $templateCache.put("decorators/material/select.html", "<md-input-container class=\"form-group {{::form.htmlClass}} schema-form-select\" md-is-error=\"ngModel.$error && ngModel.$invalid\" sf-layout=\"\" sf-material-class=\"md-input-has-value\"><label ng-show=\"::showTitle()\">{{::form.title}}</label><md-select ng-disabled=\"form.readonly\" sf-field-model=\"\" schema-validate=\"form\"><md-optgroup ng-repeat-start=\"(key, opt) in form.getOptions(form, evalExpr) | orderBy:\'group\' as optGroups\" ng-if=\"opt.group && opt.group != optGroups[key-1].group\" label=\"{{::opt.group}}\" aria-label=\"{{::opt.group}}\"><md-option-empty></md-option-empty><md-option ng-repeat=\"(key, filtered) in form.getOptions(form, evalExpr) | filter: {group: opt.group} | orderBy:\'name\' as opts\" ng-value=\"::filtered.value\" aria-label=\"{{::filtered.name}}\">{{::filtered.name}}</md-option></md-optgroup><md-option-empty ng-if=\"!opt.group\"></md-option-empty><md-option ng-if=\"!opt.group\" ng-value=\"::opt.value\" ng-repeat-end=\"\">{{::opt.name}}</md-option></md-select><div class=\"md-errors-spacer\"></div><div ng-messages=\"ngModel.$error\" aria-live=\"assertive\"><div sf-message=\"\" ng-message=\"\"></div></div><div class=\"hint md-char-counter\" ng-if=\"form.showHints\" ng-bind=\"form.description\"></div></md-input-container>");
...
(function(angular, undefined) {
    ...
    function materialDecoratorConfig(schemaFormProvider, decoratorsProvider, sfBuilderProvider, sfPathProvider, $injector) {
        ...
        decoratorsProvider.defineDecorator('materialDecorator', {
            ...
            checkboxes: { template: base + 'checkboxes.html', builder: arrays.concat(checkboxes) },
